...
def _vaapi_driver_exists(driver: str) -> bool:
    """Check if a VAAPI driver shared object exists in common locations."""
    candidates = [
        "/usr/lib/x86_64-linux-gnu/dri",
        "/usr/lib64/dri",
        "/usr/lib/dri",
    ]
    filename = f"{driver}_drv_video.so"
    return any(pathlib.Path(path, filename).is_file() for path in candidates)


def _detect_libva_driver() -> str | None:
    """Auto-detect LIBVA driver name. Returns 'iHD', 'i965', 'radeonsi', or None."""
    env_driver = os.environ.get("LIBVA_DRIVER_NAME")
    if env_driver:
        return env_driver

    vendor = _get_gpu_vendor()
    if vendor == "8086":
        if _vaapi_driver_exists("iHD"):
            return "iHD"
        if _vaapi_driver_exists("i965"):
            return "i965"
        return "i965"
    if vendor == "1002":
        return "radeonsi"
